...
This R code contains the data analysis for all results reported in Brooker et al (2025) 'Competitive cleaning: behavioural variation  supports coexistence of two juvenile  sympatric cleanerfishes' (DOI: ).
...
The chunk's below will load the data frame. You need to set your working directory to the location of the excel file, or have the the r markdown document and excel file in the same location. <br>

Import the data frame <br>
...
Changing some of the variable str <br>
...
Next, we are looking at how many trials there were. The code below will tells us how many trials we have <br>
...
Now I will use this to figure out how many of each combination we had <br>
...
Let's make some new variables. <br>
...
Making a new treatment variable that captures which species the focal fish is, as well as the competitor <br>
...
First we are going to plot the data and see what it looks like <br>
...
Using a histogram to check the disruption of the raw data <br>
...
There's lots of zeros, let's see how many there are in each species <br>
...
number_zero %>% 
  gt()
```
Number of zeros per competitive pair <br>
...
35% of the overall values are zeros <br> 
...
Because there are so many zero, we may consider testing the count data instead, and using a glmer model that handles zero inflated distributions (e.g. a zero-inflated Poisson or negative binomial) <br>

## Number of times interacting with clinet

Now we will look at the number of times interacting with the client <br>
...
This score indicates the use of the entire central compartment relative to the position of the stimulus shoal, with a higher score indicating a more social individual (maximum: 1, minimum: -1) <br>
...
Check the distribution of the data <br>
...
Checking to see if the number of interactions is correlated with assocaition score <br> 

As expected there is a significant positive correlation, fish that interact more have a higher association score (S = 9573.3, p-value < 0.001, rho = 0.734). This of makes sense, they must be close to the client to interact. It actually seems like the relationship changes depending on the species and competition context. We can explore this more with the model below. <br>
...
Building a simple linear regression <br>
...
Here we are modelling the data with a generalised linear mixed effect model <br>
...
The model is overdispered <br>

```{r}
check_overdispersion(mod_glmer)
```

We will use zero inflated and negative binomial models to see if we can improve the fit. "The error "'giveCsparse' has been deprecated" is harmless and can be ignored" (Ben Bolker, 11/04/2021) <br>
...
Fitting a negative binomial type I <br>

```{r, message=FALSE}
mod_zinbinom_1 <- update(mod_zip, family = nbinom1)
```

Fitting a negative binomial type II <br>

```{r, message=FALSE}
mod_zinbinom_2 <- update(mod_zip,family = nbinom2)
```

Fitting a Hurdle model. In contrast to zero-inflated models, hurdle models treat zero-count and nonzero outcomes as two completely separate categories, rather than treating the zero-count outcomes as a mixture of structural and sampling zeros <br>
...
We are now going to find out which model fits best <br>
...
Here we are using AICs, as a comparison. In this NB1 is the most suitble <br>
...
Visually checking model performance. Based on these visual checks the zip model seems to best approximate the residual variance much better than the other two model. <br>
...
Based on the visual checks we should use the Zero-inflated Poisson Generalized Linear Mixed Effect Model <br>
...
Here we are using a Dunn-Sidak adjustment to look at our planed comparisons, in other words we are only looking at a subset of the possible comparisons. These are Bicolor_Coris to Bluestreak_Coris, Bicolor_Bluestreak to Bluestreak_Bicolor, Bicolor_Bluestreak to Bicolor_Coris, and Bluestreak_Bicolor to Bluestreak_Coris <br>

Dunn-Sidak is a method of correcting either the alpha value or the p-value of a series of hypotheses to control the familywise error rate. It can be used for any collection of (independent) hypotheses. That is, not just to compare multiple means. <br>

Here we are pulling out specific group means of interest from the emmeans() output. We pull out a group mean by making a vector to represent the specific mean of interest. In this vector we assign a 1 to the mean of the group of interest and a 0 to the other groups <br>

The output of emmeans looks like this...<br>
...
We want to pull out Bicolor_Bluestreak, Bicolor_Coris, Bluestreak_Bicolor, and Bluestreak_Coris. To do this we make the following vectors. <br>
...
Bluestreak_Coris = c(0, 0, 0, 1, 0, 0) <br>

If we also wanted Coris_Bicolor and Coris_Bluestreak, we could do the following. <br>

Coris_Bicolor = c(0, 0, 0, 0, 1, 0)

Coris_Bluestreak = c(0, 0, 0, 0, 0, 1) <br>
...
Making a neat results table <br>
...
If you are interested, this is a list of every comparison (unadjusted) <br>
...
Let's also get the estimated mean per treatment and asymptotic lower and upper confidence limits to add to our plot <br>
...
Here's the output. Although, again, this was not the focus of the study, and probably could be estimates in a more robust way that better fits out data. Link-scale approximation: R  = 0.11, SE = 0.14, CI = [0, 0.461], P  = 0.279 [LRT] or 0.376 [Permutation] <br>
...
Building a linear mixed effect model <br>
...
Visually check model suitability. It looks pretty good. <br>
...
The random effects appear to explain a good about of variation, as the conditional R2 is quite high compared to the marginal R2 <br>
...
Checking results of the model using Type II Wald F tests with Kenward-Roger df <br>
...
I am using rpt, with a Gaussian distribution <br>
